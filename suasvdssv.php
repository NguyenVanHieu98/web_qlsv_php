...
if (!empty($_POST['suasv']))
{
    ...
    $errors = array();
    if (empty($data['mssv'])){
        $errors['mssv'] = 'Chưa nhập ma so sinh vien';
    }
    if (empty($data['hoten'])){
    ...
    if (empty($data['ngaysinh'])){
        $errors['ngaysinh'] = 'Chưa nhập ngay sinh sinh vien';
    }
    if (empty($data['quequan'])){
        $errors['quequan'] = 'Chưa nhập que quan sinh vien';
    }
    if (empty($data['malop'])){
        $errors['malop'] = 'Chưa nhập ma lop sinh vien';
    }
    if (empty($data['makhoaquanli'])){
        $errors['makhoaquanli'] = 'Chưa nhập khoa quan li sinh vien';
    }
	...
	$row1 = pg_fetch_assoc($result);
        $data1 = $row1;
        $query = pg_query($conn, " UPDATE sinhvien SET mssv = '$data[mssv]', hoten = '$data[hoten]',gioitinh = '$data[gioitinh]', ngaysinh = '$data[ngaysinh]', quequan = '$data[quequan]', malop = '$data[malop]', makhoaquanli = '$data1[makhoaquanli]'  WHERE mssv = '$mssv' ");
...
<html>
    ...
    <body> 
        <h1>Sửa sinh viên  </h1>	
        <form bgcolor=#CCC method="post" action="quantri.php?url=suasvdssv.php&mssv=<?php echo $data['mssv']; ?>">
